...
export default (Plugin) => {
  ...
  const openUI = () => {
      ...
      <Card>
        <div class="text-12" style="line-height: 1.6;">
          <div class="mb-8">
            ...
            <span>This plugin only supports importing configurations from <b>sing-box v1.12.0</b> and above。</span>
          </div>
          ...
          <div class="mb-8">
            ...
            <ul class="list-disc pl-20 opacity-80 mb-4">
              ...
              <li>
                Other fields that are not yet supported will also be automatically restored by the script at runtime.
              </li>
            </ul>
            <p class="opacity-80">
              In most cases, the runtime configuration ultimately generated by the GUI will be consistent with the original configuration you imported.
            </p>
          </div>
        </div>
      </Card>
    ...
    const modal = Plugins.modal(
      ...
      {
        default: () => h(component),
        action: () => h('div', { class: 'mr-auto text-12 opacity-60' }, 'Note: If you do not understand the above instructions, we recommend using the quick start function。')
      }
    )
    modal.open()
  }
  ...
  const importLocalConfig = async () => {
    ...
    if (!files) {
      Plugins.message.warn('No file selected')
      return
    }
    Plugins.message.info(`Starting to parse ${files.length} files...`)
    ...
    for (const [i, result] of results.entries()) {
      ...
      if (result.status === 'fulfilled') {
        try {
          ...
          Plugins.message.error(`Import of file "${fileName}" failed: ${err.message ?? String(err)}`)
        }
      } else {
    ...
    `,
      setup() {
        ...
        const importBtnText = computed(() => {
          return urlCount.value > 0 ? `Import (${urlCount.value})` : 'Start importing'
        })
        const handleImport = async () => {
          ...
          if (urls.length === 0) {
            Plugins.message.warn('No link entered')
            return
          }
...
class ConfigImporter {
  ...
  extractProxies() {
    if (!this.config.outbounds?.length) {
      Plugins.message.warn('The lack of outbound configuration may cause parsing errors.')
      return
    }
...
const fetchJson = async (url) => {
  try {
    ...
    throw `Link "${url}" failed to resolve: ${err.message ?? String(err)}`
  }
}
