...
<span class="font-bold text-primary">Formatting Requirements:</span>

<span>This plugin only supports importing configurations from <b>sing-box v1.12.0</b> and above.</b> </span>

</div>

<div class="mb-8">

<div class="font-bold text-primary mb-4">How it works:</div>

<p class="mb-4 opacity-80">

If your configuration contains settings that are not yet supported by the GUI, the plugin will handle this by dynamically generating scripts:
...
<li>
For endpoints, inbound, and DNS servers that are not yet supported, the plugin will create placeholder entries with the same names.

<div class="mt-2 text-11 italic opacity-90">

* Note: Endpoint placeholder entries will be created synchronously in both inbound and outbound scenarios.

</div>

<div class="mt-2">

To ensure the script correctly restores the configuration, <span style="color: #ff4d4f;">do not delete these placeholder items</span>, as they will be replaced with the original configuration by the script at runtime.

</div>

</li>

<li>Other <b>fields that are not yet supported</b> will also be automatically <b>restored</b> by the script at runtime.
...
In most cases, the runtime configuration ultimately generated by the GUI will <b>be consistent</b> with the original configuration you imported.</b>
...
Import from link
    ...
    const modal = Plugins.modal(
      {
        title: 'Configuration Import Help',
      ...
      {
        default: () => h(component),
        action: () => h('div', { class: 'mr-auto text-12 opacity-60' }, 'Note: If you don\'t understand the above instructions, it is recommended to use the quick start tool.')
      }
    )
    modal.open()
  }
  ...
  const importLocalConfig = async () => {
    ...
    if (!files) {
      Plugins.message.warn('No files selected')

      return

    }
    Plugins.message.info(`Start parsing ${files.length} files...`)
    ...
    for (const [i, result] of results.entries()) {
      ...
      if (result.status === 'fulfilled') {
        try {
          ...
          Plugins.message.info(`File "${fileName}" imported successfully`)

        } catch (err) {

          Plugins.message.error(`File "${fileName}" import failed: ${err.message ?? String(err)}`)
        }
      } else {
  ...
  const importRemoteConfig = () => {
    ...
    `,
      setup() {
        ...
        const importBtnText = computed(() => {
          return urlCount.value > 0 ? `Import (${urlCount.value})` : 'Start Import'
        ...
        const handleImport = async () => {
          ...
          if (urls.length === 0) {
            Plugins.message.warn('No links entered')
            return
          }
    ...
    const modal = Plugins.modal(
      ...
      {
        default: () => h(component),
        action: () => h('div', { class: 'mr-auto text-12 opacity-60' }, 'Note: Please ensure the import source is trustworthy')
      }
    )
    modal.open()
  }
...
class ConfigImporter {
  ...
  extractProxies() {
    if (!this.config.outbounds?.length) {
      Plugins.message.warn('Missing outbound configuration may cause parsing errors')
      return
    }
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        reject(`File "${file.name}" parsing failed: ${err.message ?? String(err)}`)
      }
    }
    reader.onerror = () => {
...
const fetchJson = async (url) => {
  try {
    ...
    throw `The URL "${url}" failed to be parsed: ${err.message ?? String(err)}`
  }
}
  ...
  for (const [i, result] of results.entries()) {
    ...
    if (result.status === 'fulfilled') {
      try {
        ...
        await importer.process()
        Plugins.message.info(`Link "${url}" imported successfully`)
      } catch (err) {
        failCount++
        Plugins.message.error(`Link "${url}" import failed: ${err.message ?? String(err)}`)
      }
    } else {
      failCount++
      Plugins.message.error(result.reason.message ?? result.reason)
